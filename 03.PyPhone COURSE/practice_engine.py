...
class Task:
    def __init__(self, description, expected_output=None, verify_func=None,
                 level=Level.EASY, hints=None, note=None):
...
def log_mistake(lesson_name, level, code, reason):
    notes_dir = os.path.join(os.path.dirname(__file__), "..", "04.PyPhone NOTES")
...
def _run_single(task, lesson_name="", show_timer=True):
    ...
    while True:
        if not code:
            print("⌨️  Write your code (empty line to finish):")
            code = read_multiline()
        ...
        out, err, local_ns = capture_exec(code)
